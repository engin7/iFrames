...
private let top100URL = "https://api.themoviedb.org/3/movie/top_rated"
...
// special closure takes Bool as parameter returns -> () no value to make code more readable
    func performSearch(for text: String, completion: @escaping SearchComplete )  {
        ...
        if text.isEmpty {
            ...
            for index in 1...5 {
            url = URL(string: top100URL + "?api_key=" + apiKey! + "&language=en-US&page=" + "\(index)")
    ...
    func movieDetail(for id: Int, completion: @escaping SearchComplete) {
        ...
        var searchResult: SearchResult?
        let url = URL(string: "https://api.themoviedb.org/3/movie/" + String(id) + "?api_key=" + apiKey!)
        let session = URLSession.shared
        let success = false
    ...
    // MARK:- Private Methods
      private func themoviedbURL(searchText: String) -> URL {
          ...
          let endPoint = String(format: baseURL + "?api_key=" + apiKey! + "&query=%@", encodedText) //term=%@ adds text to %@
